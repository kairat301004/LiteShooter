...
void RenderOverlayUI()
{
	...
	ss << "Patrony: " << gun->GetCurrentAmmo() << " / " << gun->GetAmmoCapacity();
	EngineUtil::DrawRasterString(font, 20, 850, ss.str());

	//glColor4ub(255, 155, 155, 255);
	EngineUtil::DrawRasterString(fontSmall, 20, 200, "Tvoya missia: ");
	EngineUtil::DrawRasterString(fontSmall, 20, 230, "Ustranit vseh vragov");
	EngineUtil::DrawRasterString(fontSmall, 20, 260, "Ustraneno: ");
...
{
	Engine::Init(argc, argv, "Shuter Kairata");
	TextureStore::Init();
